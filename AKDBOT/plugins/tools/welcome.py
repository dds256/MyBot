...
from AKDBOT.utils.daxx_ban import admin_filter
...
import asyncio
import time
from AKDBOT.utils.database import add_served_chat
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton
from AKDBOT.utils.database import get_assistant
import asyncio
from AKDBOT.misc import SUDOERS
from AKDBOT.mongo.afkdb import PROCESS
from pyrogram import Client, filters
...
from AKDBOT.utils.database import get_assistant, is_active_chat
...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("AKDBOT/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((635, 635))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('AKDBOT/assets/font.ttf', size=70)
    welcome_font = ImageFont.truetype('AKDBOT/assets/font.ttf', size=61)
...
async def add_all(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002026940573:
                continue
            try:
